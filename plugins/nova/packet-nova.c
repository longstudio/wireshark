/* packet-nova.c
 * Routines for nova protocol dissection.
 ...
 *
 * Copyright 2017, heweilong <heweilong[at]youzan.com>
 *
...
void proto_register_nova(void);
void proto_reg_handoff_nova(void);
...
/** nova header **/
static int hf_nova_msg_len = -1;
static int hf_nova_magic = -1;
static int hf_nova_header_len = -1;
static int hf_nova_version = -1;
static int hf_nova_ip = -1;
static int hf_nova_port = -1;
static int hf_nova_service_name = -1;
static int hf_nova_method_name = -1;
static int hf_nova_dubbo_request_id = -1;
static int hf_nova_attachement = -1;
/** nova header **/
...
{
    guint32 str_len = tvb_get_ntohl(tvb, offset);
    offset += 4;
...
dissect_thrift_common(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree)
{
    ...
    guint32 nova_msg_len;
    guint16 nova_magic;
    guint16 nova_header_len;
    guint8 nova_version;
    guint32 nova_ip;
    guint32 nova_port;
    guint32 nova_service_name_len;
    guint8 *nova_service_name_str;
    guint32 nova_method_name_len;
    guint8 *nova_method_name_str;
    guint64 nova_dubbo_request_id;
    guint32 nova_attachment_len;
    guint8 *nova_attachment_str;

    col_set_str(pinfo->cinfo, COL_PROTOCOL, "Nova");
    col_clear(pinfo->cinfo, COL_INFO);

    nova_msg_len = tvb_get_ntohl(tvb, offset);
    offset += 4;
    nova_magic = tvb_get_ntohs(tvb, offset);
    offset += 2;
    nova_header_len = tvb_get_ntohs(tvb, offset);
    offset += 2;
    nova_version = tvb_get_guint8(tvb, offset);
    offset += 1;
    nova_ip = tvb_get_ntohl(tvb, offset);
    offset += 4;
    nova_port = tvb_get_ntohl(tvb, offset);
    offset += 4;
    nova_service_name_len = tvb_get_ntohl(tvb, offset);
    offset += 4;
    nova_service_name_str = tvb_get_string_enc(wmem_packet_scope(), tvb, offset, nova_service_name_len, ENC_UTF_8);
    offset += nova_service_name_len;
    nova_method_name_len = tvb_get_ntohl(tvb, offset);
    offset += 4;
    nova_method_name_str = tvb_get_string_enc(wmem_packet_scope(), tvb, offset, nova_method_name_len, ENC_UTF_8);
    offset += nova_method_name_len;
    nova_dubbo_request_id = tvb_get_ntoh64(tvb, offset);
    offset += 8;
    nova_attachment_len = tvb_get_ntohl(tvb, offset);
    offset += 4;
    ...
    sub_tree = proto_tree_add_subtree_format(tree, tvb, 0, -1, ett_thrift, NULL, "%s[ reqid:%ld, service:%s, method:%s]",
        ...
        nova_dubbo_request_id,
        nova_service_name_str,
        nova_method_name_str);

    col_add_fstr(pinfo->cinfo, COL_INFO, "%s %s.%s", val_to_str(mtype, thrift_mtype_vals, "%d"), nova_service_name_str, nova_method_name_str);

    if (tree){
        offset = 0;
        proto_tree_add_item(sub_tree, hf_nova_msg_len, tvb, offset, 4, ENC_BIG_ENDIAN);
        offset += 4;
        proto_tree_add_item(sub_tree, hf_nova_magic, tvb, offset, 2, ENC_BIG_ENDIAN);
        offset += 2;
        proto_tree_add_item(sub_tree, hf_nova_header_len, tvb, offset, 2, ENC_BIG_ENDIAN);
        offset += 2;
        proto_tree_add_item(sub_tree, hf_nova_version, tvb, offset, 1, ENC_BIG_ENDIAN);
        offset += 1;
        proto_tree_add_item(sub_tree, hf_nova_ip, tvb, offset, 4, ENC_BIG_ENDIAN);
        offset += 4;
        proto_tree_add_item(sub_tree, hf_nova_port, tvb, offset, 4, ENC_BIG_ENDIAN);
        offset += 4;
        offset += 4;
        proto_tree_add_item(sub_tree, hf_nova_service_name, tvb, offset, nova_service_name_len, ENC_ASCII | ENC_NA);
        offset += nova_service_name_len;
        offset += 4;
        proto_tree_add_item(sub_tree, hf_nova_method_name, tvb, offset, nova_method_name_len, ENC_ASCII | ENC_NA);
        offset += nova_method_name_len;
        proto_tree_add_item(sub_tree, hf_nova_dubbo_request_id, tvb, offset, 8, ENC_BIG_ENDIAN);
        offset += 8;
        offset += 4;
...
dissect_thrift_heur(tvbuff_t* tvb, packet_info* pinfo, proto_tree* tree, void *data _U_) {
    ...
    /* Need at least 54 bytes for a nova message */
    if (length < 54){
        return FALSE;
    }

    nova_msg_len = tvb_get_ntohl(tvb, offset);
    if (nova_msg_len != length) {
        return FALSE;
    }
...
static void
convertToIp(gchar *buff, guint32 val)
{
	int ipInt = (int) ((guint32) val);
	g_snprintf(buff, ITEM_LABEL_LENGTH, "%d.%d.%d.%d", ((ipInt >> 24) & 0xff), 
...
void proto_register_nova(void) {
    ...
    static hf_register_info hf[] = {
        { &hf_nova_msg_len,
        { "Nova Message Length", "nova.msg_len",
        FT_UINT32, BASE_DEC, NULL, 0x0,
        NULL, HFILL }
        },
        { &hf_nova_magic,
        { "Nova Magic", "nova.magic",
        FT_UINT16, BASE_HEX, NULL, 0x0,
        NULL, HFILL }
        },
        { &hf_nova_header_len,
        { "Nova Header Length", "nova.header_length",
        ...
        { &hf_nova_ip,
        { "Nova Ip", "nova.ip",
        FT_UINT32, BASE_CUSTOM, convertToIp, 0x0,
        NULL, HFILL }
        },
        { &hf_nova_port,
        { "Nova Port", "nova.port",
        ...
        { &hf_nova_service_name,
        { "Nova Service Name", "nova.service_name",
        FT_STRING, BASE_NONE, NULL, 0x0,
        NULL, HFILL }
        },
        { &hf_nova_method_name,
        { "Nova Method Name", "nova.method_name",
        FT_STRING, BASE_NONE, NULL, 0x0,
        NULL, HFILL }
        },
        { &hf_nova_dubbo_request_id,
        { "Nova Dubbo Request Id", "nova.dubbo_request_id",
        FT_STRING, BASE_NONE, NULL, 0x0,
        NULL, HFILL }
        },
        { &hf_nova_attachement,
        { "Nova Attachment", "nova.attachement",
        FT_STRING, BASE_NONE, NULL, 0x0,
        NULL, HFILL }
        },
        { &hf_thrift_version,
    ...
    proto_thrift = proto_register_protocol("Nova Protocol", "Nova", "nova");
...
void proto_reg_handoff_nova(void) {
    ...
    heur_dissector_add("tcp", dissect_thrift_heur, "Nova over TCP", "nova_tcp", proto_thrift, HEURISTIC_ENABLE);
    thrift_http_handle = create_dissector_handle(dissect_thrift_heur, proto_thrift);
    dissector_add_string("media_type", "application/x-nova", thrift_http_handle);
